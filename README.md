...
├── index.html                    ← the working engine; open this to run the prototype
├── about.html                    ← project landing page (safe for GitHub Pages)
├── engine/
│   ├── visible_trusted_engine.html ← the engine (same as index.html; standalone copy)
│   └── visible_trusted_engine.jsx  ← the engine as a React component (runs as a Claude artifact)
├── generator/
...
**To just run the prototype:** open `index.html` in a browser (it is the engine, self-contained, no build step). The flow, the instant rule-layer, the gates, and the distilled brief all work anywhere. The one caveat is the **live AI critique**: it only reaches the model from inside Claude's runtime. Opened straight from disk or GitHub Pages, the engine falls back to a clearly-labelled local check so the whole experience is still demonstrable.
